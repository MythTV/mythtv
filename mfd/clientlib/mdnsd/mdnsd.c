...
{ // call the answer function with this cached entry
    if(c->rr.ttl <= (unsigned int) d->now.tv_sec) c->rr.ttl = 0;
...
{ // expire any old entries in this list
    ...
    {
        next = cur->next;
        if( (unsigned int) d->now.tv_sec >= cur->rr.ttl)
        {
...
{ // copy a published record into an outgoing message
    ...
    while((r = *list) != 0 && message_packet_len(m) + _rr_len(&r->rr) < d->frame)
    {
        *list = r->list;
        ret++;
        if(r->unique)
            message_an(m, r->rr.name, r->rr.type, d->a_class + 32768, r->rr.ttl);
        else
            message_an(m, r->rr.name, r->rr.type, d->a_class, r->rr.ttl);
        _a_copy(m, &r->rr);
...
mdnsd mdnsd_new(int a_class, int frame)
{
    //int i;
    mdnsd d;
...
int mdnsd_out(mdnsd d, struct message *m, unsigned long int *ip, unsigned short int *port)
{
    ...
    *ip = inet_addr("224.0.0.251");
    ...
    { // send out individual unicast answers
        ...
        *ip = u->to;
        m->id = u->id;
        message_qd(m, u->r->rr.name, u->r->rr.type, d->a_class);
        message_an(m, u->r->rr.name, u->r->rr.type, d->a_class, u->r->rr.ttl);
        _a_copy(m, &u->r->rr);
        free(u);
        return 1;
    }
...
//printf("OUT: probing %X now %X pause %X publish %X\n",d->probing,d->a_now,d->a_pause,d->a_publish);
    ...
    { // check to see if it's time to send the publish retries (and unlink if done)
        ...
        while(cur && message_packet_len(m) + _rr_len(&cur->rr) < d->frame)
        {
            ...
            ret++; cur->tries++;
            if(cur->unique)
                message_an(m, cur->rr.name, cur->rr.type, d->a_class + 32768, cur->rr.ttl);
            else
                message_an(m, cur->rr.name, cur->rr.type, d->a_class, cur->rr.ttl);
            _a_copy(m, &cur->rr);
    ...
    if(d->probing && _tvdiff(d->now,d->probe) <= 0)
    {
        ...
        { // scan probe list to ask questions and process published
            ...
            message_qd(m, r->rr.name, r->rr.type, d->a_class);
            last = r;
            r = r->list;
        }
        ...
        { // scan probe list again to append our to-be answers
            r->unique++;
            message_ns(m, r->rr.name, r->rr.type, d->a_class, r->rr.ttl);
            _a_copy(m, &r->rr);
            ret++;
        }
        if(ret)
    ...
    if(d->checkqlist && (unsigned int) d->now.tv_sec >= d->checkqlist)
    { // process qlist for retries or expirations
        ...
        for(q = d->qlist; q != 0; q = q->list)
            if(q->nexttry > 0 && q->nexttry <= (unsigned int) d->now.tv_sec && q->tries < 3)
                message_qd(m,q->name,q->type,d->a_class);
        ...
        for(q = d->qlist; q != 0; q = q->list)
        {
            if(q->nexttry == 0 || q->nexttry > (unsigned int) d->now.tv_sec) continue;
            if(q->tries == 3)
    ...
    if((unsigned int) d->now.tv_sec > d->expireall)
        _gc(d);
