...
my @types = ('cclocation', 'station_id', 
...
if ($getData) {
    my $base_url = 'http://www.weatheroffice.gc.ca/forecast/city_e.html?';
    my $response = get $base_url . $loc;
...
if ($units eq "ENG") { 
	...
	$results{'wind_spdgst'} = sprintf("%.2f (%.2f) mph", $results{'wind_speed'}, $results{'wind_gust'});
	
	for (my $i=0;$i<6;$i++) {
        if ($results{"high-$i"} ne "NA") {
        	$results{"high-$i"} = int(((9/5) * $results{"high-$i"}) + 32);
        }
        if ($results{"low-$i"} ne "NA") {
