...
void GlobalSetup::wireUI() {
    ...
    if (!blckhl) {
        ...
        connect(blckhl, SIGNAL(takingFocus()), m_timeout_spinbox, SLOT(setFocus()));
        /* loosing focus? */
        connect(blckhl, SIGNAL(loosingFocus()), m_timeout_spinbox, SLOT(clearFocus()));
    }
    ...
    if (!blckhl) {
        ...
        blckhl->allowFocus(true);
        connect(blckhl, SIGNAL(takingFocus()), m_hold_spinbox, SLOT(setFocus()));
        connect(blckhl, SIGNAL(loosingFocus()), m_hold_spinbox, SLOT(clearFocus()));
    }
...
void GlobalSetup::saveData() {
    ...
    gContext->SaveSetting("weatherbackgroundfetch", m_background_check->getState() ? 1 : 0);
...
void GlobalSetup::keyPressEvent(QKeyEvent *e) {
    ...
    for (uint i = 0; i < actions.size() && !handled; ++i) {
        ...
        if (action == "DOWN" ) {
            ...
            UICheckBoxType* check;
            if (check = dynamic_cast<UICheckBoxType*> (curr)) {
                check->push();
            }
        } else if (action == "MENU") {
            saveData();
...
void ScreenSetup::wireUI() {
    ...
    if (!m_active_list) {
        ...
        connect(m_active_list, SIGNAL(takingFocus()), this, SLOT(updateHelpText()));
        connect(m_active_list, SIGNAL(itemSelected(UIListBtnTypeItem*)),
    ...
    if (!m_inactive_list) {
        ...
        connect(m_inactive_list, SIGNAL(takingFocus()), this, SLOT(updateHelpText()));
    }
    ...
    if (!m_type_list) {
        ...
        //connect(m_active_list, SIGNAL(takingFocus()), m_type_list, SLOT(show()));
        connect(m_type_list, SIGNAL(takingFocus()), m_type_list, SLOT(show()));
        connect(m_active_list, SIGNAL(loosingFocus()), m_type_list, SLOT(hide()));
        connect(m_active_list, SIGNAL(takingFocus()), 
                this, SLOT(activeListItemSelected()));
        connect(m_type_list, SIGNAL(takingFocus()), this, SLOT(updateHelpText()));
        connect(m_type_list, SIGNAL(itemSelected(UIListBtnTypeItem*)), this, SLOT(updateHelpText()));
    }
...
void ScreenSetup::keyPressEvent(QKeyEvent *e) {
    ...
    for (uint i = 0; i < actions.size() && !handled; ++i) {
        ...
        if (action == "DOWN" ) {
            ...
            cursorLeft(curr);   
        } else if (action == "SEARCH" && (list = dynamic_cast<UIListBtnType*> (curr))) {
            list->incSearchStart();   
            updateForeground(list->getScreenArea());
        } else if (action == "NEXTSEARCH" && (list = dynamic_cast<UIListBtnType*> (curr))) {
            list->incSearchNext();   
            updateForeground(list->getScreenArea());
        } else handled = false;
...
void ScreenSetup::updateHelpText() {
    ...
    if (itm == m_inactive_list) {
        ...
        struct ScreenListInfo* si = 
            (struct ScreenListInfo*) m_active_list->GetItemCurrent()->getData();
        QDictIterator<struct TypeListInfo> it(si->types);
        struct TypeListInfo* ti = it.current();
        text += m_active_list->GetItemCurrent()->text() + "\n";
        if (si->hasUnits) 
            text += 
                tr("Units: ") + (si->units == ENG_UNITS ? tr("English Units") : tr("SI Units")) + "\n";
        if (!si->multiLoc && ti) { 
            text += tr("Location: ") + (ti->location != "" ? ti->location : tr("Not Defined")) + "\n"; 
            text += tr("Source: " ) + (ti->src ? ti->src->name : tr("Not Defined")) + "\n";
        }
        text += "\n" + tr("Press SELECT to ");
        if (!si->multiLoc)
        ...
        if (btnitm) {
            struct TypeListInfo *ti = (struct TypeListInfo*) btnitm->getData();
            text = tr("%1\nLocation: %2\nSource: %3\n\nPress SELECT to change Location settings")
                .arg(btnitm->text()).arg(ti->location != "" ? ti->location : tr("Not Defined"))
...
void ScreenSetup::loadData(){
    ...
    QIntDict<struct ScreenListInfo> active_screens;
    struct ScreenListInfo* si; 
    struct TypeListInfo* ti;
    ...
    while (!n.isNull()) {
        ...
        //        if (!active_screens.find(name)) {
                    UIListBtnTypeItem* itm = new UIListBtnTypeItem(m_inactive_list, name);
               ...
               // } else {
               //   struct ScreenListInfo * si = active_screens[name];
               //     si->hasUnits = !(bool) set->GetType("nounits");
               //     si->multiLoc = (bool) set->GetType("multilocation");
               // }
            }
...
void ScreenSetup::saveData(){
    ...
    while(an_it) {
       ...
       struct ScreenListInfo *si = (struct ScreenListInfo*) itm->getData();
...
void ScreenSetup::doListSelect(UIListBtnType* list, UIListBtnTypeItem* selected) {
    ...
    if (list == m_active_list) {
        struct ScreenListInfo* si = (struct ScreenListInfo*) selected->getData();
        QStringList buttons;
        if (!si->multiLoc) 
        ...
        buttons << tr("Move Up") << tr("Move Down") << tr("Remove") << tr("Cancel");
        QString result = buttons[MythPopupBox::showButtonPopup(gContext->GetMainWindow(), "Manipulate Screen",
                tr("Action to take on screen ") + selected->text(), buttons, buttons.count()-1)];
        if (result == tr ("Change Location")) {
            doLocationDialog(si, true);
        } else if (result == tr("Change Units")) {
                showUnitsPopup(selected->text(), (struct ScreenListInfo*) selected->getData());
                updateHelpText();
        ...
        QDictIterator<struct TypeListInfo> it(si->types);
        for (; it.current(); ++it) {
            struct TypeListInfo* newti = new TypeListInfo;
            memcpy(newti, it.current(), sizeof(struct TypeListInfo));
        ...
        QPtrList<struct ScriptInfo> tmp;
        if (m_src_man->findPossibleSources(type_strs, tmp)) {
            struct ScreenListInfo* newsi = new ScreenListInfo;
            newsi->units = si->units;
            newsi->types.setAutoDelete(true);
            newsi->types = types;
            newsi->hasUnits = si->hasUnits;
            newsi->multiLoc = si->multiLoc;
            //delete selected;
            if (!list->GetCount()) {
                list->allowFocus(false);
                nextPrevWidgetFocus(true);
            ...
            MythPopupBox::showOkPopup(gContext->GetMainWindow(), "Add Screen Error", 
                    tr("Screen cannot be used, not all required data "
                        "is supplied by existing sources"));
        }
    } else if (list == m_type_list) {
        doLocationDialog( (struct ScreenListInfo*) m_active_list->GetItemCurrent()->getData(), false);
    } 

}
void ScreenSetup::doLocationDialog( struct ScreenListInfo* si,  bool alltypes){
    /*
     * If its alltypes, we round up all types for this screen,
     * if its not, just the seleted item in m_type_list
     */
    QStringList types;
    QPtrList<struct TypeListInfo> infos;
    if (alltypes) {
        QDictIterator<struct TypeListInfo> it(si->types);
        for (; it.current(); ++it) {
            struct TypeListInfo* ti = it.current();
        ...
        struct TypeListInfo* ti = (struct TypeListInfo*) m_type_list->GetItemCurrent()->getData();
        infos.append(ti);
        types << ti->name;
    }
    QString loc;
    ...
    if (showLocationPopup(types, loc, src)){
        for (struct TypeListInfo* ti = infos.first(); ti; ti = infos.next()) {
            ti->location = loc;
            ti->src = src;
        }
        updateHelpText();
...
void ScreenSetup::activeListItemSelected(UIListBtnTypeItem* itm) {
    ...
    QDict<struct TypeListInfo> types = si->types;
    m_type_list->Reset();
    UITextType* txt = getUITextType("typelbl");
    if (si->multiLoc) {
        QDictIterator<struct TypeListInfo> it(si->types);
        for (; it.current(); ++it) {
            ...
            new UIListBtnTypeItem(m_type_list, it.current()->name);
            itm->setData(it.current());
        }
    if (txt) txt->show();
    m_type_list->show();
    } else {
...
bool ScreenSetup::showUnitsPopup(QString name, struct ScreenListInfo* si) {
    ...
    switch(MythPopupBox::showButtonPopup(gContext->GetMainWindow(), "Change Units",
...
bool ScreenSetup::showLocationPopup(QStringList types, QString &loc, struct ScriptInfo* &src) {
...
void ScreenSetup::cursorUp(UIType* curr) {
    UIListBtnType* list;
    if (list = dynamic_cast<UIListBtnType*> (curr)) {
...
void ScreenSetup::cursorRight(UIType* curr) {
    if (curr == m_active_list) {
        ...
        if (((struct ScreenListInfo*) itm->getData())->multiLoc) {
            buildFocusList();
            nextPrevWidgetFocus(true);
        }
    } 
...
void ScreenSetup::cursorMenu() {
    ...
    if (notDefined.size()) {
        ...
        msg += notDefined.join("\n");
        MythPopupBox::showOkPopup(gContext->GetMainWindow(), "Undefined Sources", msg);
        return;
    }
    saveData();
    accept();
...
SourceSetup::~SourceSetup() {
    ...
    while ( itm = it.current() ) {
        if (itm->getData()) 
            delete (struct SourceListInfo*) itm->getData();
        ++it;
    }
}
...
void SourceSetup::wireUI() {
    ...
    if (!blckhl) {
        ...
        connect(blckhl, SIGNAL(takingFocus()), m_update_spinbox, SLOT(setFocus()));
        connect(blckhl, SIGNAL(loosingFocus()), m_update_spinbox, SLOT(clearFocus()));
        connect(blckhl, SIGNAL(loosingFocus()), this, SLOT(updateSpinboxUpdate()));
    }
    ...
    if (!blckhl) {
        ...
        connect(blckhl, SIGNAL(takingFocus()), m_retrieve_spinbox, SLOT(setFocus()));
        connect(blckhl, SIGNAL(loosingFocus()), m_retrieve_spinbox, SLOT(clearFocus()));
        connect(blckhl, SIGNAL(loosingFocus()), this, SLOT(retrieveSpinboxUpdate()));
    }
}
...
bool SourceSetup::loadData() {
    ...
    while (db.next()) {
        struct SourceListInfo* si = new SourceListInfo;
...
void SourceSetup::saveData() {
    ...
    struct SourceListInfo* si = (struct SourceListInfo*) m_src_list->GetItemCurrent()->getData();
    ...
    QString query = "UPDATE weathersourcesettings "
        "SET update_timeout = :UPDATE, retrieve_timeout = :RETRIEVE WHERE sourceid = :ID;";
    db.prepare(query);

    QPtrListIterator<UIListBtnTypeItem> an_it = m_src_list->GetIterator();

    while(an_it) {
        struct SourceListInfo* si = (struct SourceListInfo*) (*an_it)->getData();
...
void SourceSetup::sourceListItemSelected(UIListBtnTypeItem* itm) {
    ...
    struct SourceListInfo* si = (struct SourceListInfo*) itm->getData();
    if (!si)
        return;
...
LocationDialog::LocationDialog(MythMainWindow* parent, QStringList types, SourceManager *srcman)
...
void LocationDialog::doSearch() {
    QMap<struct ScriptInfo*, QStringList> result_cache;
    int numresults = 0;
    m_list->Reset();
    ...
    for (uint i = 0; i < result_cache.keys().size(); ++i) {
        struct ScriptInfo* si = result_cache.keys()[i];
        ...
        for (uint ii = 0; ii < results.size(); ++ii) {
            ...
            struct ResultListInfo* ri = new ResultListInfo;
            ri->idstr = tmp[0];
            ri->src = si;
            itm->setData(ri);
        }
...
void LocationDialog::keyPressEvent(QKeyEvent *e) {
    ...
    for (uint i = 0; i < actions.size() && !handled; ++i) {
        ...
        if (action == "DOWN") {
            ...
            m_list->MoveDown(UIListBtnType::MoveMax); updateForeground(m_list->getScreenArea());
...
void LocationDialog::itemSelected(UIListBtnTypeItem* itm) {
    ...
    struct ResultListInfo* ri = (struct ResultListInfo*)itm->getData();
    if (ri) 
...
struct ScriptInfo* LocationDialog::getSource() {
    UIListBtnTypeItem* itm = m_list->GetItemCurrent();
    if (!itm)
        return NULL;
