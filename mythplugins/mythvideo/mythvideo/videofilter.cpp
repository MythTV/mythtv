...
QString VideoFilterSettings::BuildClauseFrom(){
	...
	if (genre!=-1)
		if (genre==0)
			from = QString("( %1 LEFT JOIN videometadatagenre ON videometadata.intid = videometadatagenre.idvideo)").arg(from);
		else
			from = QString("( %1 INNER JOIN videometadatagenre ON videometadata.intid = videometadatagenre.idvideo)").arg(from);
	
	if (country!=-1)
		if (country==0)
			from = QString("( %1 LEFT JOIN videometadatacountry ON videometadata.intid = videometadatacountry.idvideo)").arg(from);
		else
			from = QString("( %1 INNER JOIN videometadatacountry ON videometadata.intid = videometadatacountry.idvideo)").arg(from);
	return from;
}
...
QString VideoFilterSettings::BuildClauseOrderBy(){
	switch (orderby){
		...
		case 1 : return " ORDER BY year DESC";
		case 2 : return " ORDER BY userrating DESC";
		case 3 : return " ORDER BY length";
	}
	return "";
}
...
{
     if (originalSettings){
//        originalSettings->setShowlevel(currentSettings->getShowlevel());
