		...
		if section =='series_name_override':
			...
			for key in overrides.keys():
				...
				try: # Check that the SID (Series id) is numeric
					...
					sys.stdout.write("! Series (%s) Invalid SID (not numeric) [%s]\n" % (key, sid))
					sys.exit(False)
				# Make sure that the series name is not empty or all blanks
				if len(key.replace(' ','')) == 0:
					sys.stdout.write("! Invalid Series name (must have some non-blank characters) [%s]\n" % key)
					print parts
					sys.exit(False)
				...
				try:
					...
					sys.stdout.write("! Invalid Series (no matches found in thetvdb,com) (%s) sid (%s)\n" % (key, sid))
					sys.exit(False)
