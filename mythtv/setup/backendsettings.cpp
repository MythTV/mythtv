    ...
    virtual public Configurable
{
  public:
    ClearChannels()
    {
        setLabel(QObject::tr("Clear Channels"));
        setHelpText(QObject::tr(
                        "Would you like to clear all program data "
                        "and channel settings before starting \n"
                        "configuration? This will not affect any "
                        "existing recordings.")+" ");
    }
};
...
{
    VerticalConfigurationGroup *grp = new VerticalConfigurationGroup();

    TransLabelSetting *backendLabel = new TransLabelSetting();
    ...
    if (is_backend_running())
        addChild(backendLabel);

    ccard = new ClearCards();
    TransLabelSetting *ccardLabel = new TransLabelSetting();
    ccardLabel->setLabel(ccard->getHelpText());
    grp->addChild(ccardLabel);
    grp->addChild(ccard);

    cchan = new ClearChannels();
    TransLabelSetting *cchanLabel = new TransLabelSetting();
    cchanLabel->setLabel(cchan->getHelpText());
    grp->addChild(cchanLabel);
    grp->addChild(cchan);

    addChild(grp);

    okbtn = new TransButtonSetting();
    okbtn->setLabel(QObject::tr("OK"));

    addChild(okbtn);
}

bool ClearDialog::IsClearCardsSet(void) const
{
    return ccard->boolValue();
}

bool ClearDialog::IsClearChannelsSet(void) const
{
    return cchan->boolValue();
}

int ClearDialog::exec(void)
{
    MythDialog* dialog = dialogWidget(gContext->GetMainWindow());
    connect(okbtn, SIGNAL(pressed()), dialog, SLOT(accept()));
