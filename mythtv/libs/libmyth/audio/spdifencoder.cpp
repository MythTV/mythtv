    ...
    : m_complete(false), m_oc(NULL), m_stream(NULL), m_size(0)
{
    ...
    {
        LOG(VB_AUDIO, LOG_ERR, "av_set_parameters");
        Destroy();
        return;
    }
    ...
    if (!m_oc->pb)
    {
        LOG(VB_AUDIO, LOG_ERR, "av_alloc_put_byte");
        Destroy();
        return;
    }
    ...
    if (!m_stream)
    {
        LOG(VB_AUDIO, LOG_ERR, "av_new_stream");
        Destroy();
        return;
    }
    ...
    LOG(VB_AUDIO, LOG_INFO, QString("Creating %1 encoder (for %2)")
