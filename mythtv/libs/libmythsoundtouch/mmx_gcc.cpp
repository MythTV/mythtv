...
long TDStretchMMX::calcCrossCorrStereo(const short *pV1, const short *pV2) const
{
    ...
    GI(__m64 m0, m1, m2, m3, m4); // temporaries
    ...
    do {
        ...
        SI(m2 = _mm_srai_pi32(m2, shift), psrad_m2r(shift, mm2));    // m2 >>= shift
        ...
        SI(m3 = _mm_srai_pi32(m3, shift), psrad_m2r(shift, mm3));    // m3 >>= shift
    ...
    SI(m2 = _mm_srai_pi32(m2, shift),  psrad_m2r(shift, mm2));
    SI(m3 = _mm_add_pi32(m3, m4),      paddd_r2r(mm4, mm3));
    SI(m0 = _mm_add_pi32(m0, m2),      paddd_r2r(mm2, mm0));
    SI(m3 = _mm_srai_pi32(m3, shift),  psrad_m2r(shift, mm3));
...
void TDStretchMMX::overlapStereo(short *output, const short *input) const
{
    ...
    __m64 *outPtr = ((__m64*) output)-2;             // load address of midBuffer
    ...
    do {
        ...
        SI(m1 = m0,                       movq_r2r(mm0, mm3));      // mm1 = m1l m1r m0l m0r
        SI(m2 = midPtr[1],                movq_a2r(8, midPtr, mm2));// mm2 = m3l m3r m2l m2r
        SI(m0 = _mm_unpacklo_pi16(m0, m3),punpckldq_r2r(mm3, mm0)); // mm0 = i0l m0l i0r m0r
        midPtr += 2;
        SI(m4 = inPtr[1],                 movq_a2r(8, inPtr, mm4)); // mm4 = i3l i3r i2l i2r
        SI(m1 = _mm_unpackhi_pi16(m1, m3),punpckldq_r2r(mm3, mm1)); // mm1 = i1l m1l i1r m1r
        inPtr+=2;
        SI(m3 = m2,                       movq_r2r(mm2, mm3));      // mm3 = m3l m3r m2l m2r
        SI(m2 = _mm_unpacklo_pi16(m2, m4),punpckldq_r2r(mm4, mm2)); // mm2 = i2l m2l i2r m2r
        // mm0 = i0l*m63+m0l*m62 i0r*m61+m0r*m60
        SI(m0 = _mm_madd_pi16(m0, m6),    pmaddwd_r2r(mm6, mm0));
        SI(m3 = _mm_unpackhi_pi16(m3, m4),punpckldq_r2r(mm4, mm3)); // mm3 = i3l m3l i3r m3r
        ...
        SI(m0 = _mm_srai_pi32(m0, shift), psrad_m2r(shift, mm0));  // mm0 >>= shift
        // mm2 = i2l*m63+m2l*m62 i2r*m61+m2r*m60
        SI(m2 = _mm_madd_pi16(m2, m6),    pmaddwd_r2r(mm6, mm2));
        SI(m1 = _mm_srai_pi32(m1, shift), psrad_m2r(shift, mm1));  // mm1 >>= shift
        // mm3 = i3l*m73+m3l*m72 i3r*m71+m3r*m70
        SI(m3 = _mm_madd_pi16(m3, m7),    pmaddwd_r2r(mm7, mm3));
        SI(m2 = _mm_srai_pi32(m2, shift), psrad_m2r(shift, mm2));  // mm2 >>= shift
        SI(m0 = _mm_packs_pi32(m0, m1),   packssdw_r2r(mm1, mm0)); // mm0 = mm1h mm1l mm0h mm0l
        SI(m3 = _mm_srai_pi32(m3, shift), psrad_m2r(shift, mm3));  // mm3 >>= shift
        SI(m6 = _mm_add_pi16(m6, m5),     paddw_r2r(mm5, mm6));
...
uint FIRFilterMMX::evaluateFilterStereo(short *dest, const short *src, const uint numSamples) const
{
    ...
    do {
        ...
        SI(m0 = _mm_srai_pi32(m0, resultDivFactor), psrad_m2r(resultDivFactor, mm0));
        outPtr++;
        // Divide mm7 by 8192 (= right-shift by 13)
        SI(m7 = _mm_srai_pi32(m7, resultDivFactor), psrad_m2r(resultDivFactor, mm7));
        inPtr++;
