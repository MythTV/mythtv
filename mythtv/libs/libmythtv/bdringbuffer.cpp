...
{
    LOG(VB_FILE, LOG_INFO, QString("Seek(%1,%2,%3)")
    ...
    else
    {
        QString cmd = QString("Seek(%1, %2)").arg(pos)
            .arg((whence == SEEK_SET) ? "SEEK_SET" :
                 ((whence == SEEK_CUR) ?"SEEK_CUR" : "SEEK_END"));
        LOG(VB_GENERAL, LOG_ERR, cmd + " Failed" + ENO);
    }
...
{
    LOG(VB_PLAYBACK, LOG_INFO, QString("Seeking to %1.").arg(pos));
    if (bdnav)
...
bool BDRingBuffer::OpenFile(const QString &lfilename, uint retry_ms)
{
    ...
    LOG(VB_GENERAL, LOG_INFO, QString("Opened BDRingBuffer device at %1")
    ...
    {
        LOG(VB_GENERAL, LOG_INFO, QString("Disc Title: %1 (%2)")
                    .arg(m_metaDiscLibrary->di_name)
                    .arg(m_metaDiscLibrary->language_code));
        LOG(VB_GENERAL, LOG_INFO, QString("Alternative Title: %1")
                    .arg(m_metaDiscLibrary->di_alternative));
        LOG(VB_GENERAL, LOG_INFO, QString("Disc Number: %1 of %2")
    ...
    if (discinfo)
    {
        ...
        LOG(VB_PLAYBACK, LOG_INFO, QString("*** Blu-ray Disc Information ***"));
        LOG(VB_PLAYBACK, LOG_INFO, QString("First Play Supported: %1")
                .arg(discinfo->first_play_supported ? "yes" : "no"));
        LOG(VB_PLAYBACK, LOG_INFO, QString("Top Menu Supported: %1")
                .arg(discinfo->top_menu_supported ? "yes" : "no"));
        LOG(VB_PLAYBACK, LOG_INFO, QString("Number of HDMV Titles: %1")
                .arg(discinfo->num_hdmv_titles));
        LOG(VB_PLAYBACK, LOG_INFO, QString("Number of BD-J Titles: %1")
                .arg(discinfo->num_bdj_titles));
        LOG(VB_PLAYBACK, LOG_INFO, QString("Number of Unsupported Titles: %1")
                .arg(discinfo->num_unsupported_titles));
        LOG(VB_PLAYBACK, LOG_INFO, QString("AACS present on disc: %1")
                .arg(discinfo->aacs_detected ? "yes" : "no"));
        LOG(VB_PLAYBACK, LOG_INFO, QString("libaacs used: %1")
                .arg(discinfo->libaacs_detected ? "yes" : "no"));
        LOG(VB_PLAYBACK, LOG_INFO, QString("AACS handled: %1")
                .arg(discinfo->aacs_handled ? "yes" : "no"));
        LOG(VB_PLAYBACK, LOG_INFO, QString("BD+ present on disc: %1")
                .arg(discinfo->bdplus_detected ? "yes" : "no"));
        LOG(VB_PLAYBACK, LOG_INFO, QString("libbdplus used: %1")
                .arg(discinfo->libbdplus_detected ? "yes" : "no"));
        LOG(VB_PLAYBACK, LOG_INFO, QString("BD+ handled: %1")
    ...
    LOG(VB_GENERAL, LOG_INFO, QString("Using %1 as keyfile...")
            .arg(QString(keyfilepath)));

    // Return an index of relevant titles (excludes dupe clips + titles)
    LOG(VB_GENERAL, LOG_INFO, QString("Retrieving title list (please wait)."));
    m_numTitles = bd_get_titles(bdnav, TITLES_RELEVANT, 30);
    LOG(VB_GENERAL, LOG_INFO, QString("Found %1 titles.").arg(m_numTitles));
    m_mainTitle = 0;
    ...
    {
        LOG(VB_GENERAL, LOG_INFO, QString("Using HDMV navigation mode."));
    ...
    {
        LOG(VB_GENERAL, LOG_INFO, QString("Using title navigation mode."));
...
bool BDRingBuffer::SwitchPlaylist(uint32_t index)
{
    ...
    LOG(VB_PLAYBACK, LOG_INFO, "SwitchPlaylist - start");
    ...
    LOG(VB_PLAYBACK, LOG_INFO, "SwitchPlaylist - end");
    return result;
}
...
BLURAY_TITLE_INFO* BDRingBuffer::GetTitleInfo(uint32_t index)
{
    ...
    if (result)
    {
        LOG(VB_PLAYBACK, LOG_INFO, QString("Found title %1 info").arg(index));
...
bool BDRingBuffer::UpdateTitleInfo(void)
{
    ...
    {
        LOG(VB_PLAYBACK, LOG_INFO, "Entering infinite still frame.");
    }
...
int BDRingBuffer::GetAudioLanguage(uint streamID)
{
    ...
    LOG(VB_GENERAL, LOG_INFO, QString("Audio Lang: %1 Code: %2")
...
{
    LOG(VB_PLAYBACK, LOG_INFO, QString("Key %1 (pts %2)").arg(key).arg(pts));
...
bool BDRingBuffer::GoToMenu(const QString str, int64_t pts)
{
    ...
    {
        LOG(VB_PLAYBACK, LOG_INFO, "Top Menu not supported");
        return false;
    }

    LOG(VB_PLAYBACK, LOG_INFO, QString("GoToMenu %1").arg(str));

    if (str.compare("root") == 0)
    {
        if (bd_menu_call(bdnav, pts))
        {
            LOG(VB_PLAYBACK, LOG_INFO, QString("Invoked Top Menu (pts %1)")
                                                .arg(pts));
            return true;
        }
    }
...
void BDRingBuffer::HandleBDEvent(BD_EVENT &ev)
{
    switch (ev.event) {
        case BD_EVENT_NONE:
            break;
        case BD_EVENT_ERROR:
            LOG(VB_PLAYBACK, LOG_INFO, QString("EVENT_ERROR %1").arg(ev.param));
            break;
        case BD_EVENT_ENCRYPTED:
            LOG(VB_GENERAL, LOG_ERR, "EVENT_ENCRYPTED, playback will fail.");
            break;

        /* current playback position */

        case BD_EVENT_ANGLE:
            LOG(VB_PLAYBACK, LOG_INFO, QString("EVENT_ANGLE %1").arg(ev.param));
            m_currentAngle = ev.param;
            break;
        case BD_EVENT_TITLE:
            LOG(VB_PLAYBACK, LOG_INFO, QString("EVENT_TITLE %1 (old %2)")
                                .arg(ev.param).arg(m_currentTitle));
            m_currentTitle = ev.param;
            break;
        case BD_EVENT_END_OF_TITLE:
            LOG(VB_PLAYBACK, LOG_INFO, QString("EVENT_END_OF_TITLE %1")
                                .arg(m_currentTitle));
            WaitForPlayer();
            break;
        case BD_EVENT_PLAYLIST:
            LOG(VB_PLAYBACK, LOG_INFO, QString("EVENT_PLAYLIST %1 (old %2)")
        ...
        case BD_EVENT_PLAYITEM:
            LOG(VB_PLAYBACK, LOG_INFO, QString("EVENT_PLAYITEM %1")
                                .arg(ev.param));
            m_currentPlayitem = ev.param;
            break;
        case BD_EVENT_CHAPTER:
            // N.B. event chapter numbering 1...N, chapter seeks etc 0...
            LOG(VB_PLAYBACK, LOG_INFO, QString("EVENT_CHAPTER %1")
                                .arg(ev.param));
            m_currentChapter = ev.param;
            break;
        case BD_EVENT_STILL:
            LOG(VB_PLAYBACK, LOG_INFO, QString("EVENT_STILL %1").arg(ev.param));
            break;
        case BD_EVENT_STILL_TIME:
            ...
            break;
        case BD_EVENT_SEEK:
            LOG(VB_PLAYBACK, LOG_INFO, QString("EVENT_SEEK"));
            break;

        /* stream selection */

        case BD_EVENT_AUDIO_STREAM:
            LOG(VB_PLAYBACK, LOG_INFO, QString("EVENT_AUDIO_STREAM %1")
                                .arg(ev.param));
            m_currentAudioStream = ev.param;
            break;
        case BD_EVENT_IG_STREAM:
            LOG(VB_PLAYBACK, LOG_INFO, QString("EVENT_IG_STREAM %1")
                                .arg(ev.param));
            m_currentIGStream = ev.param;
            break;
        case BD_EVENT_PG_TEXTST_STREAM:
            LOG(VB_PLAYBACK, LOG_INFO, QString("EVENT_PG_TEXTST_STREAM %1")
                                .arg(ev.param));
        ...
        case BD_EVENT_PG_TEXTST:
            LOG(VB_PLAYBACK, LOG_INFO, QString("EVENT_PG_TEXTST %1")
                                .arg(ev.param ? "enable" : "disable"));
            m_PGTextSTEnabled = ev.param;
            break;
        case BD_EVENT_SECONDARY_AUDIO:
            LOG(VB_PLAYBACK, LOG_INFO, QString("EVENT_SECONDARY_AUDIO %1")
                                .arg(ev.param ? "enable" : "disable"));
            m_secondaryAudioEnabled = ev.param;
            break;
        case BD_EVENT_SECONDARY_VIDEO:
            LOG(VB_PLAYBACK, LOG_INFO, QString("EVENT_SECONDARY_VIDEO %1")
                                .arg(ev.param ? "enable" : "disable"));
            m_secondaryVideoEnabled = ev.param;
            break;
        case BD_EVENT_SECONDARY_VIDEO_SIZE:
            LOG(VB_PLAYBACK, LOG_INFO, QString("EVENT_SECONDARY_VIDEO_SIZE %1")
                                .arg(ev.param==0 ? "PIP" : "fullscreen"));
            m_secondaryVideoIsFullscreen = ev.param;
            break;

        default:
            LOG(VB_PLAYBACK, LOG_ERR, QString("Unknown Event! %1 %2")
...
void BDRingBuffer::WaitForPlayer(void)
{
    ...
    LOG(VB_PLAYBACK, LOG_INFO, "Waiting for player's buffers to drain");
    m_playerWait = true;
    int count = 0;
    ...
    if (m_playerWait)
    {
        LOG(VB_GENERAL, LOG_ERR, "Player wait state was not cleared");
        m_playerWait = false;
    }
}
...
bool BDRingBuffer::StartFromBeginning(void)
{
    ...
    {
        LOG(VB_PLAYBACK, LOG_INFO, "Starting from beginning...");
...
void BDRingBuffer::SubmitOverlay(const bd_overlay_s * const overlay)
{
    ...
    {
        LOG(VB_GENERAL, LOG_ERR, QString("Invalid overlay size: %1x%2+%3+%4")
