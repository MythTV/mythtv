...
bool DiSEqCDevTree::Store(uint cardid)
{
    ...
    {
        LOG(VB_GENERAL, LOG_ERR, "Failed to save DiSEqC tree.");
        return false;
    }
...
bool DiSEqCDevTree::SetTone(bool on)
{
    ...
    if (!success)
        LOG(VB_GENERAL, LOG_ERR, "FE_SET_TONE failed" + ENO);
                            ...
                            const DTVMultiplex      &tuning)
{
    if (!m_root)
    {
        LOG(VB_GENERAL, LOG_ERR, "No root device tree node!");
        return false;
    }
                                ...
                                uint data_len, unsigned char *data)
{
    ...
    {
        LOG(VB_GENERAL, LOG_ERR, "Bad DiSEqC command");
        return false;
    }
    ...
    LOG(VB_CHANNEL, LOG_INFO, "Sending DiSEqC Command: " + cmdstr);
    ...
    for (uint i = 0; i <= repeats; i++)
    {
        ...
        {
            LOG(VB_GENERAL, LOG_ERR, "DiSEqC command failed" + ENO);
            return false;
        }
...
bool DiSEqCDevTree::ResetDiseqc(bool hard_reset)
{
    ...
    if (hard_reset)
    {
        LOG(VB_CHANNEL, LOG_INFO, "Power-cycling DiSEqC Bus");
    ...
    LOG(VB_CHANNEL, LOG_INFO, "Resetting DiSEqC Bus");
    if (!SendCommand(DISEQC_ADR_ALL, DISEQC_CMD_RESET))
    {
        LOG(VB_GENERAL, LOG_ERR, "DiSEqC reset failed" + ENO);
        return false;
    }
...
bool DiSEqCDevTree::SetVoltage(uint voltage)
{
    ...
    LOG(VB_CHANNEL, LOG_INFO, "Changing LNB voltage to " +
    ...
    if (!success)
    {
        LOG(VB_GENERAL, LOG_ERR, "FE_SET_VOLTAGE failed" + ENO);
        return false;
    }
...
DiSEqCDevDevice *DiSEqCDevDevice::CreateById(DiSEqCDevTree &tree, uint devid)
{
    ...
    {
        LOG(VB_GENERAL, LOG_ERR, "CreateById failed to find dtv dev " +
                QString("%1").arg(devid));
                              ...
                              const DTVMultiplex      &tuning)
{
    ...
    if (ShouldSwitch(settings, tuning))
    {
        switch (m_type)
        {
            ...
            default:
                success = false;
                LOG(VB_GENERAL, LOG_ERR, QString("Unknown switch type (%1)")
                        .arg((uint)m_type));
                break;
        }
        ...
        {
            LOG(VB_CHANNEL, LOG_INFO, "Waiting for switch");
            usleep(DISEQC_LONG_WAIT);
        }
                                    ...
                                    uint pos)
{
    ...
    LOG(VB_CHANNEL, LOG_INFO, QString("Changing to Legacy switch port %1/%2")
    ...
    if (ioctl(m_tree.GetFD(), FE_DISHNETWORK_SEND_LEGACY_CMD,
              cmds[pos] | horizcmd) == -1)
    {
        LOG(VB_GENERAL, LOG_ERR, "FE_DISHNETWORK_SEND_LEGACY_CMD failed" + ENO);
    ...
    LOG(VB_GENERAL, LOG_ERR, "You must compile with a newer "
...
static bool set_tone(int fd, fe_sec_tone_mode tone)
{
    ...
    if (!success)
    {
        LOG(VB_GENERAL, LOG_ERR, "set_tone failed" + ENO);
    }
...
bool DiSEqCDevSwitch::ExecuteTone(const DiSEqCDevSettings &/*settings*/,
                                  ...
                                  uint pos)
{
    LOG(VB_CHANNEL, LOG_INFO, "Changing to Tone switch port " +
    ...
    LOG(VB_GENERAL, LOG_ERR, "Setting Tone Switch failed." + ENO);
    return false;
}
                                     ...
                                     const DTVMultiplex &tuning, uint pos)
{
    ...
    LOG(VB_CHANNEL, LOG_INFO, "Changing to Voltage Switch port " +
            QString("%1/2").arg(pos + 1));
    ...
    LOG(VB_GENERAL, LOG_ERR, "Setting Voltage Switch failed." + ENO);
                                        ...
                                        const DTVMultiplex &tuning, uint pos)
{
    ...
    LOG(VB_CHANNEL, LOG_INFO, "Changing to MiniDiSEqC Switch port " +
            QString("%1/2").arg(pos + 1));
    ...
    LOG(VB_GENERAL, LOG_ERR, "Setting Mini DiSEqC Switch failed." + ENO);
                                    ...
                                    uint pos)
{
    ...
    LOG(VB_CHANNEL, LOG_INFO, "Changing to DiSEqC switch port " +
...
int DiSEqCDevSwitch::GetPosition(const DiSEqCDevSettings &settings) const
{
    ...
    {
        LOG(VB_GENERAL, LOG_ERR, QString("Port %1 ").arg(pos + 1) +
                             ...
                             const DTVMultiplex &tuning)
{
    ...
    if (m_reset || (position != m_last_position))
    {
        switch (m_type)
        {
            ...
            default:
                success = false;
                LOG(VB_GENERAL, LOG_ERR, "Unknown rotor type " +
                                  ...
                                  double angle)
{
    ...
    LOG(VB_CHANNEL, LOG_INFO, "USALS Rotor - " +
