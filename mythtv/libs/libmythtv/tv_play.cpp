...
void TV::timerEvent(QTimerEvent*)
{
    TVEventThreadChecks();
}
...
{
    QMutexLocker locker(&udpnotifyLock);
...
void TV::HandleUDPNotifyEvent(void)
{
    ...
    udpnotifyLock.unlock();

    if (GetOSD() && set)
    {
        GetOSD()->StartNotify(set);
    }
    else if (GetOSD() && !name.isEmpty())
    {
        GetOSD()->ClearNotify(name);
    }
}
