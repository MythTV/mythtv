...
"{\n"
"    highp float temp = xy.y * m_frameData.w;\n"
"    highp float onetwo = min((floor(temp / 2.0) / (m_frameData.w / 2.0)) + m_frameData.x, m_frameData.z);\n"
"    return vec2(xy.x, mix(onetwo, min(onetwo + (2.0 * m_frameData.x), m_frameData.z), step(0.5, fract(temp))));\n"
"}\n"
"#endif\n"
