...
void MasterGuideTable::Parse() const {
    _ptrs.clear();
...
void EventInformationTable::Parse() const {
    _ptrs.clear();
    _ptrs.push_back(const_cast<unsigned char*>(pesdata())+11);
...
QString MasterGuideTable::toString() const {
    QString str;
    ...
    for (unsigned int i=0; i<TableCount(); i++) {
        ...
        str.append(QString("size(%1) desc_len(%2) type: %4 %3 \n").
                   arg(TableDescriptorsBytes(i), 4, 10).arg(TableDescriptorsLength(i)).
                   arg(TableClassString(i)).arg(TableType(i)));
        if (0!=TableDescriptorsLength(i)) {
            vector<const unsigned char*> desc = 
                ATSCDescriptor::Parse(TableDescriptors(i), TableDescriptorsLength(i));
...
QString TerrestrialVirtualChannelTable::toString() const {
    QString str;
...
QString EventInformationTable::toString() const {
    ...
    for (uint i=0; i<EventCount(); i++) {
        str.append(QString(" Event #%1 ID(%2) start_time(%3) length(%4 sec)\n").
                   ...
                   arg(StartTimeGPS(i).toString(Qt::LocalDate)).arg(LengthInSeconds(i)));
...
QString ExtendedTextTable::toString() const {
    QString str = 
        QString("Extended Text Table -- sourceID(%1) eventID(%2) ettID(%3) isChannelETM(%4) isEventETM(%5)\n").
