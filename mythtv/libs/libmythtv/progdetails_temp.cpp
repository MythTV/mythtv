...
ProgDetailsTemp::ProgDetailsTemp(MythScreenStack *parent, const ProgramInfo *progInfo)
...
bool ProgDetailsTemp::Create(void)
{
    ...
    float zoom = gContext->GetSetting("ProgDetailsTempZoom", "1.0").toFloat();
    m_browser->SetZoom(zoom);

    return true;
}

QString ProgDetailsTemp::getRatings(bool recorded, uint chanid, QDateTime startts)
{
    ...
    {
        MythDB::DBError("ProgDetailsTemp::getRatings", query);
        return "";
    }
...
void ProgDetailsTemp::Init()
{
    updatePage();
}

ProgDetailsTemp::~ProgDetailsTemp(void)
{
    float zoom = m_browser->GetZoom();
    gContext->SaveSetting("ProgDetailsTempZoom", QString().setNum(zoom));
}

bool ProgDetailsTemp::keyPressEvent(QKeyEvent *event)
{
...
void ProgDetailsTemp::updatePage(void)
{
...
void ProgDetailsTemp::addItem(const QString &key, const QString &title, const QString &data)
{
...
void ProgDetailsTemp::removeItem(const QString &key)
{
...
void ProgDetailsTemp::loadPage(void)
{
    loadHTML();
    ...
    if (m_progInfo.endts != m_progInfo.startts)
    {
        ...
        if (query.exec() && query.isActive() && query.size() > 0)
        {
            ...
            MythDB::DBError("ProgDetailsTemp", query);
...
bool ProgDetailsTemp::loadHTML(void)
{
    m_html.clear();
...
void ProgDetailsTemp::showMenu(void)
{
...
void ProgDetailsTemp::customEvent(QEvent *event)
{
