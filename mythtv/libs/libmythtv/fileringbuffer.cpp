    ...
    if (is_local)
    {
        ...
        switch (lasterror)
        {
            ...
            case 1:
                ...
                lastError = QObject::tr("Could not open %1").arg(filename);
                break;
            case 2:
                LOG(VB_GENERAL, LOG_ERR, LOC +
                        QString("OpenFile(): File too small (%1B).")
                        .arg(QFileInfo(filename).size()));
                lastError = QObject::tr("File too small (%1B)")
                            .arg(QFileInfo(filename).size());
                break;
            case 3:
                LOG(VB_GENERAL, LOG_ERR, LOC +
                        "OpenFile(): Improper permissions.");
                lastError = QObject::tr("Improper permissions");
                break;
            case 4:
                LOG(VB_GENERAL, LOG_ERR, LOC +
                        "OpenFile(): Cannot seek in file.");
                lastError = QObject::tr("Cannot seek in file");
                break;
            default:
                break;
        }
        ...
        if (!remotefile->isOpen())
        {
            LOG(VB_GENERAL, LOG_ERR, LOC +
                    QString("RingBuffer::RingBuffer(): Failed to open remote "
                            "file (%1)").arg(filename));
            lastError = QObject::tr("Failed to open remote file %1").arg(filename);
            delete remotefile;
            remotefile = NULL;
        }
        else
