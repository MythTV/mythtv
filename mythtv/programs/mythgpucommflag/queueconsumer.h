...
{
  public:
    QueueConsumer(PacketQueue *inQ, ResultsMap *outMap, OpenCLDevice *dev,
                  QString name) : 
        MThread(name), m_name(name), m_inQ(inQ), m_outMap(outMap),
        m_done(false), m_dev(dev), m_context(NULL), m_codec(NULL),
        m_stream(NULL), m_opened(false) {};
    ~QueueConsumer() {};
