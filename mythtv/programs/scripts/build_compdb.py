...
for path in paths:
    ...
    if any(bad in filename for bad in badnames):
        if options.verbose:
            print("Skipping input file: {:s}".format(path.as_posix()))
        continue
    in_file = open(path)
    if not in_file:
        if options.verbose:
            print("Cannot open input file: {:s}".format(filename))
        continue

    # Read and strip trailing comma
    rawcontent = in_file.read()
    if len(rawcontent) < 2:
        print("Bad file contents: {:s}".format(filename))
        exit(1)
    ...
    if content["file"].startswith("../") and not content["file"].endswith("Json.cpp"):
        if options.verbose:
            print("Skipping input file2: {:s}".format(filename))
        continue
    content["file"] = content["directory"] + "/" + content["file"]

    # Save the output for this file
    if options.verbose:
        print("Including input file: {:s}".format(path.as_posix()))
