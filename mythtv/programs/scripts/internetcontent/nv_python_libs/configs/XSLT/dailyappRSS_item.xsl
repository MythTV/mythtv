...
<!--
    The Daily App Show items conversion to MythNetvision item format
-->
<xsl:stylesheet version="1.0"
    ...
    -->
    <xsl:template match="/">
        <xsl:if test="//channel/link='http://dailyappshow.com'">
            <xml>
                <xsl:call-template name='dailyappshow'/>
            </xml>
        </xsl:if>
    </xsl:template>

    <xsl:template name='dailyappshow'>
