...
<!--
    Terra items conversion to MythNetvision item format
-->
<xsl:stylesheet version="1.0"
    ...
    -->
    <xsl:template match="/">
        <xsl:if test="//channel/link='http://www.lifeonterra.com'">
            <xml>
                <xsl:call-template name='terra'/>
            </xml>
        </xsl:if>
    </xsl:template>

    <xsl:template name='terra'>
        <xsl:for-each select='//item'>
            <dataSet>
                <xsl:if test="../image/url">
                    <directoryThumbnail><xsl:value-of select="normalize-space(../image/url)"/></directoryThumbnail>
                </xsl:if>
                <xsl:if test="not(../image/url)">
