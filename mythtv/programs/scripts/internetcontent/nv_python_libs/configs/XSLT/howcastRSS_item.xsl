...
<!--
    Howcast Tech & Gadgets items conversion to MythNetvision item format
-->
<xsl:stylesheet version="1.0"
    ...
    -->
    <xsl:template match="/">
        <xsl:if test="//channel/link='http://www.howcast.com'">
            <xml>
                <xsl:call-template name='howcast'/>
            </xml>
        </xsl:if>
    </xsl:template>

    <xsl:template name='howcast'>
