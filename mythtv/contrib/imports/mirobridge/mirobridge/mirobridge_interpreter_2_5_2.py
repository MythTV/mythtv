...
class MiroInterpreter(cmd.Cmd):
    ...
    def do_mythtv_update_autodownload(self, line):
        ...
        # priority
        eventloop.addTimeout(10, startup.clear_icon_cache_orphans, "clear orphans")
    ...
    def do_mythtv_updatewatched(self, line):
        ...
        for video in self.videofiles:
            ...
            if self.simulation:
                ...
                it.mark_item_seen(markOtherItems=True)
    ...
    def do_mythtv_getunwatched(self, line):
        ...
        self.videofiles = []
        if item.Item.newly_downloaded_view().count():
            if self.verbose:
                print u"%-20s %-10s %s" % (u"State", u"Size", u"Name")
                print u"-" * 70
            for it in item.Item.newly_downloaded_view():
    ...
    def do_mythtv_item_remove(self, args):
        ...
        for it in item.Item.downloaded_view():
             if isinstance(args, list):
                  if filter(self.is_not_punct_char, it.get_channel_title(True).lower()) == filter(self.is_not_punct_char, args[0].lower()) and filter(self.is_not_punct_char, it.get_title().lower()) == filter(self.is_not_punct_char, args[1].lower()):
                     break
             elif filter(self.is_not_punct_char, it.get_filename().lower()) == filter(self.is_not_punct_char, args.lower()):
                 break
        else:
