...
class MiroInterpreter(cmd.Cmd):
    ...
    def do_mythtv_check_downloading(self, line):
        ...
        for it in downloadingItems:
            logging.info("(%s - %s) video is downloading with (%0.0f%%) complete" % (it.get_channel_title(True).replace('/','-'), it.get_title().replace('/','-'), it.download_progress()))
        if not count:
    ...
    def do_mythtv_import_opml(self, filename):
        ...
        try:
            ...
            logging.info("Import of OPML file (%s) sent to Miro" % (filename))
        except Exception, e:
            logging.info("Import of OPML file (%s) failed, error (%s)" % (filename, e))
    ...
    def do_mythtv_updatewatched(self, line):
        ...
        for video in self.videofiles:
            for it in items:
                ...
                logging.info("Item for Miro video (%s) not found, skipping" % video)
                continue
            if self.simulation:
                logging.info("Simulation: Item (%s - %s) marked as seen and watched" % (it.get_channel_title(True), it.get_title()))
            else:
                it.mark_item_seen(mark_other_items=True)
                self.statistics['Miro_marked_watch_seen']+=1
                logging.info("Item (%s - %s) marked as seen and watched" % (it.get_channel_title(True), it.get_title()))
    ...
    def do_mythtv_getunwatched(self, line):
        ...
        if item.Item.unwatched_downloaded_items().count():
            ...
            for it in item.Item.unwatched_downloaded_items():
                ...
                #if not it.isVideo: ##### This field has been removed from an item in Miro v2.5.2
                #    continue
                state = it.get_state()
                ...
                if it.get_filename() is None:
                    ...
                    logging.info('Unwatched video (%s) has been removed from Miro as item had no valid file name' % it.get_title())
                    continue
        ...
        if item.Item.downloaded_view().count():
            ...
            for it in item.Item.downloaded_view():
                ...
                if it.get_filename() is None:
                    it.expire()
                    self.statistics['Miro_videos_deleted']+=1
                    logging.info('Watched video (%s) has been removed from Miro as item had no valid file name' % it.get_title())
                    continue
...
#            print
    ...
    def do_mythtv_item_remove(self, args):
        ...
        if it.is_downloaded():
            if self.simulation:
                logging.info("Simulation: Item (%s - %s) has been removed from Miro" % (it.get_channel_title(True), it.get_title()))
            else:
                it.expire()
                self.statistics['Miro_videos_deleted']+=1
    ...
    def _get_item_dict(self, it):
        ...
        def useImageMagick(screenshot):
            ...
            width_height = re.compile('''^(.+?)[ ]\\[?([0-9]+)x([0-9]+)[^\\/]*$''', re.UNICODE)
            p = subprocess.Popen('identify "%s"' % (screenshot), shell=True, bufsize=4096, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, close_fds=True)
        ...
        item_dict =  {'feed_id': it.feed_id, 'parent_id': it.parent_id, 'isContainerItem': it.isContainerItem, 'seen': it.seen, 'autoDownloaded': it.autoDownloaded, 'pendingManualDL': it.pendingManualDL, 'downloadedTime': it.downloadedTime, 'watchedTime': it.watchedTime, 'pendingReason': it.pendingReason, 'title': title,  'expired': it.expired, 'keep': it.keep, 'videoFilename': it.get_filename(), 'eligibleForAutoDownload': it.eligibleForAutoDownload, 'duration': it.duration, 'screenshot': it.screenshot, 'resumeTime': it.resumeTime, 'channelTitle': channel_title, 'description': it.get_description(), 'size': it._get_size(), 'releasedate': it.get_release_date(), 'length': it.get_duration_value(), 'channel_icon': channel_icon, 'item_icon': item_icon_filename, 'inetref': '', 'season': 1, 'episode': 1,}
