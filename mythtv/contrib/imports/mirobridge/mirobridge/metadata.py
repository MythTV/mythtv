...
class MetaData(object):
    ...
    def __init__(self,
        ...
        self.ttvdb = tvdb_api.Tvdb(banners=False, debug = False, cache = self.cache_dir, custom_ui=None, language = 'en', apikey="0BB856A59C51D607")
        self.makeRecordRule = {'chanid': channel_id, 'description': 'Mirobridge Record rule used to acquire ttvdb.com artwork',
                               'autometadata': 1, 'season': 1, 'episode': 1, 'category': 'Miro', 'station': 'Miro'  }
    # end __init__()
    ...
    def getMetadata(self, title):
        ...
        """
        ttvdbGraphics = {'inetref': '', 'coverart': '', 'banner': '', 'fanart': '', } # Initalize dictionary of graphics
        ...
        if not len(recordedRules_array):
            ...
            if inetref is not None:     # Create a new rule for this Miro Channel title
                ...
                if self.simulation:
                    self.logger.info("Simulation: Create Recoding rule for Title(%s) inetref(%s)" % (self.makeRecordRule['title'], self.makeRecordRule['inetref'], ))
                else:
                    record = self.Record().create(self.makeRecordRule)
                    if self.verbose:
                        self.logger.info("Created Recoding rule for Title(%s) inetref(%s)" % (self.makeRecordRule['title'], self.makeRecordRule['inetref'], ))
        ...
        if not ttvdbGraphics['coverart']:
            result = glob.glob('%s%s Season*_coverart.*' % (self.vid_graphics_dirs['posterdir'], title, ))
            if len(result):
    ...
    def convertOldMiroVideos(self):
        ...
        """
        videometadata = list(self.mythdb.searchVideos(category='Miro', custom=(('inetref=%s','99999999'),)))
        ...
        for record in videometadata:
            ...
            if not record['host'] or record['filename'][0] == '/':
                ...
                filename = self.vid_graphics_dirs['mythvideo']+record['filename']
            ...
            record['category'] = 'Video Cast'
            if self.simulation:
                self.logger.info("Simulation: Converting old copied Miro video Title(%s) file(%s)" % (record['title'], record['filename'], ))
            else:
                self.Video(record['intid'], db=self.mythdb).update(record)
                if self.verbose:
                    self.logger.info("Converting old copied Miro video Title(%s) file(%s)" % (record['title'], record['filename'], ))
        return
    ...
    def cleanupVideoAndGraphics(self, fileName):
        ...
        try:
            results = glob.glob('%s/%s*' % (dirName, fileBaseName, ))
        ...
        if len(results):
            for result in results:
                try:
                    if self.simulation:
                        self.logger.info("Simulation: Remove file/symbolic link(%s)" % result)
                    else:
                        os.remove(result)
    ...
    def deleteFile(self, filename, host, groupKey):
        ...
        if filename[0] == '/':
            try:
                if self.simulation:
                    self.logger.info("Simulation: Remove of a file (%s)" % (filename))
                else:
                    os.remove(filename)
            except OSError:
                pass
            ...
            try:
                if self.simulation:
                    self.logger.info("Simulation: Remove of a file (%s)" % (self.storagegroups[groupKey]+filename))
                else:
    ...
    def takeScreenShot(self, videofile, screenshot_filename, size_limit=False, just_demensions=False):
        ...
        if not ffmpeg_details:
            return None
        elif not 'width' in ffmpeg_details.keys():
            return None

        max_length = 2*60    # Maximum length of a video check if set at 2 minutes
        if ffmpeg_details['duration']:
            if ffmpeg_details['duration'] < max_length:
                delay = (ffmpeg_details['duration']/2) # Maximum time to take screenshot into a video is 1 minute
            else:
                delay = 60 # For a large videos take screenshot at the 1 minute mark

        cmd = 'ffmpeg -i "%s" -y -f image2 -ss %d -sameq -vframes 1 -s %d*%d "%s"'

        width = int(ffmpeg_details['width'])
        height = int(ffmpeg_details['height'])

        if size_limit:
            width = 320
            height = (int(width * ffmpeg_details['aspect'])/2)*2 # ((xxxxx)/2) *2

        if just_demensions:
            return "%dx%d" % (width, height)

        cmd2 = cmd % (videofile, delay, width, height, screenshot_filename)
        return subprocess.call('%s > /dev/null 2>/dev/null' % cmd2, shell=True)
    # end takeScreenShot()
    ...
    def getVideoDetails(self, videofilename, screenshot=False):
        ...
        '''
        ffmpeg_details = {'video': '', 'audio': '', 'duration': 0, 'stereo': 0, 'hdtv': 0}
        ...
        video_HDTV_small = re.compile(' 1280x', re.UNICODE)
        video_HDTV_large = re.compile(' 1920x', re.UNICODE)
        width_height = re.compile('''^(.+?)\\[?([0-9]+)x([0-9]+)\\,[^\\/]''', re.UNICODE)

        audio = re.compile(' Audio: ', re.UNICODE)
        audio_stereo = re.compile(' stereo,', re.UNICODE)
        audio_mono = re.compile(' mono,', re.UNICODE)
        audio_ac3 = re.compile(' ac3,', re.UNICODE)
        audio_51 = re.compile(' 5.1,', re.UNICODE)
        audio_2C = re.compile(' 2 channels,', re.UNICODE)
        audio_1C = re.compile(' 1 channels,', re.UNICODE)
        audio_6C = re.compile(' 6 channels,', re.UNICODE)

        try:
            p = subprocess.Popen('ffmpeg -i "%s"' % (videofilename), shell=True, bufsize=4096, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, close_fds=True)
        except:
            if not screenshot:
                return ffmpeg_details
            else:
        ...
        while 1:
            ...
            try:
                ...
                pass
            if data.endswith('command not found\n'):
                ffmpeg_found = False
                break
            if data.startswith('  Duration:'):
                time = (data[data.index(':')+1: data.index('.')]).strip()
                ffmpeg_details['duration'] = (360*(int(time[:2]))+(int(time[3:5])*60))+int(time[6:8])
                datacount+=1
                ...
                if match:
                    ...
                    aspect = height/width
                    if screenshot:
                        ffmpeg_details['width'] = width
                        ffmpeg_details['height'] = height
                        ffmpeg_details['aspect'] = aspect
                    if width > 1300.0 or width > 800.0 or width == 720.0 or width == 1080.0 :
                        ffmpeg_details['video']+='HDTV'
                        ffmpeg_details['hdtv'] = 1
                    elif aspect <= 0.5625:
                        ffmpeg_details['video']+='WIDESCREEN'
                    if len(ffmpeg_details['video']):
                        comma = ','
                    else:
                        comma = ''
                    if width > 1300.0:
                        ffmpeg_details['video']+=comma+'1080'
                    elif width > 800.0:
                        ffmpeg_details['video']+=comma+'720'
            elif len(audio.findall(data)):
                datacount+=1
                if len(audio_stereo.findall(data)) or len(audio_2C.findall(data)):
                    ffmpeg_details['audio']+='STEREO'
                    ffmpeg_details['stereo'] = 1
                elif len(audio_mono.findall(data)) or len(audio_1C.findall(data)):
                    ffmpeg_details['audio']+='MONO'
                elif (len(audio_51.findall(data)) and len(audio_ac3.findall(data))) or len(audio_6C.findall(data)):
                    ffmpeg_details['audio']+='DOLBY'
                    continue
                elif len(audio_51.findall(data)):
                    ffmpeg_details['audio']+='SURROUND'
