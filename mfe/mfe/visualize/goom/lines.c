...
genline (int id, float param, GMUnitPointer * l, int rx, int ry)
{
	...
	case GML_CIRCLE:
		for (i = 0; i < 256; i++) {
			...
			l[i].angle = 2.0f * M_PI * (float) i / 256.0f;
								 ...
								 int IDdest, float paramD, int coulD)
{
	...
	l->points = (GMUnitPointer *) malloc (256 * sizeof (GMUnitPointer));
	l->points2 = (GMUnitPointer *) malloc (256 * sizeof (GMUnitPointer));
	l->nbPoints = 256;
...
void
goom_lines_draw (GMLine * line, gint16 data[256], unsigned int *p)
{
	if (line != NULL) {
		int     i, x1, y1;
